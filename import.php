...
<script>
    $(function(){ $("#save2db").click(function() {
        if(validateTable())
        {
...
<script>
    $(function(){ $("#marks2db").click(function() {
        if(validateTable())
        {
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
...
<script>
    function Generate_Marks_Table(xml) {
        ...
        for (i = 0; i <x.length; i++) {
            ...
            if (batch.length === 11) {
                grade = batch.slice(0,3); batch = batch.slice(6);
            } else {
                grade = batch.slice(0,4); batch = batch.slice(7);
            }
...
<script>
        ...
        for (i = 0; i <x.length; i++) {
            ...
            fixname = fixname.replace("'","");  fixname = fixname.replace("'","");
            sqlid += "'" + fixname + "',";
...
$(function(){ $("#importDetails").click(function() {
        var iurl = $("#iurl").val();        var token = $("#token").val();              var admissionStr;
        var grade = document.getElementById('grade').value;
        var batch = document.getElementById('batch').value;                 out.value = "";
